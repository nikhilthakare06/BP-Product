*CLASS lcl_jptbupaassign_helper IMPLEMENTATION.
*
*  METHOD get_db_delta.
...
*ENDCLASS.
*&---------------------------------------------------------------------*
*& Class (Implementation) lcl_jptbupaassign_helper
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
CLASS lcl_bpassign_helper IMPLEMENTATION.
  METHOD get_db_delta.
    CLEAR: et_zmm_jptbupaassign_insert, et_zmm_jptbupaassign_update, "et_zmm_jptbupaassign_update_old,
          et_zmm_jptbupaassign_delete.
    ...
    LOOP AT gt_zmm_jptbupaassign_mem INTO ls_zmm_jptbupaassign_u.
      ...
      READ TABLE gt_zmm_jptbupaassign_db INTO DATA(ls_zmm_jptbupaassign_db) WITH KEY "main_key COMPONENTS
        matnr    = ls_zmm_jptbupaassign_u-matnr
        partner  = ls_zmm_jptbupaassign_u-partner
        rltyp    = ls_zmm_jptbupaassign_u-rltyp
        order_nr = ls_zmm_jptbupaassign_u-order_nr.
      ...
      IF sy-subrc EQ 0." AND ls_zmm_jptbupaassign_u-xmainidcode NE ls_zmm_jptbupaassign_db-xmainidcode.
        INSERT ls_zmm_jptbupaassign_u INTO TABLE et_zmm_jptbupaassign_update.
*        INSERT ls_zmm_jptbupaassign_db INTO TABLE et_zmm_jptbupaassign_update_old.
      ENDIF.
    ENDLOOP.
    ...
    LOOP AT gt_zmm_jptbupaassign_db INTO ls_zmm_jptbupaassign_db.
      ...
      IF NOT line_exists( gt_zmm_jptbupaassign_mem[  matnr = ls_zmm_jptbupaassign_db-matnr
                                                     partner = ls_zmm_jptbupaassign_db-partner
                                                     rltyp = ls_zmm_jptbupaassign_db-rltyp
                                                     order_nr  = ls_zmm_jptbupaassign_db-order_nr ] ).
